...
if FORMAT:match 'html' then
  ...
  function CodeBlock(block)
    ...
    -- check for 'linenumbers' in lang
    if string.find(lang, 'linenumbers') then
      linenos = 'inline'
      if string.find(lang, 'start=') then
        linenostart = string.match(lang, 'start=(%d+)')
      end
    end

    --strip lang of 'linenumbers', 'start=', and 'step='
    lang = string.gsub(lang, 'linenumbers', '')
    lang = string.gsub(lang, 'start=%d+', '')
    
    cmd = string.format('pygmentize -l %s -f %s -P linenos=%s -P linenostart=%s < %s', 
        lang, 'html', linenos, linenostart, filename)
    -- print(cmd)
